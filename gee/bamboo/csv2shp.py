...
shp_fn="BambooSamples.shp"#即将转化为的shapefile文件名
...
Bamboo_lyr=shp_ds.CreateLayer(shp_fn,sr,ogr.wkbPoint)#创建一个点图层
tag_id=ogr.FieldDefn('tag_id',ogr.OFTString)#为点图层创建字段
timestamp=ogr.FieldDefn('timestamp',ogr.OFTString)
Bamboo_lyr.CreateField(tag_id)
Bamboo_lyr.CreateField(timestamp)
for csv_row in csv_lyr:#对于csv文件中每一行
    point_feature=ogr.Feature(Bamboo_lyr.GetLayerDefn())#创建一个点
    ...
    Bamboo_lyr.CreateFeature(point_feature)#将点写入到图层中
del ds

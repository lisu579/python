...
import os
path=r'D:\qq\BambooSamples.csv'
os.chdir(os.path.dirname(path))#将path所在的目录设置为当前文件夹
shp_fn="Bamboo_shp.shp"#即将转化为的shapefile文件名
...
tag_id=ogr.FieldDefn('ID',ogr.OFTString)#为点图层创建字段
timestamp=ogr.FieldDefn('class',ogr.OFTString)
...
for csv_row in csv_lyr:#对于csv文件中每一行
    ...
    shp_pt.AddPoint(x,y)
    #获取字段值，并写入到该点中
    tag_value=csv_row.GetFieldAsString('ID')
    timestamp_value=csv_row.GetFieldAsString('class')
    point_feature.SetField('ID',tag_value)
    point_feature.SetField('class',timestamp_value)
